...
export default function CampaignCustomizePage() {
    ...
    const { state } = useLocation() as { state?: { template?: any } };
    ...
    const templateId = sp.get("template") ?? chosen?.id ?? "custom";
    const backTo = sp.get("back") ?? "/campaign/create-template";
    ...
    React.useEffect(() => {
        ...
        setEmail((prev) => ({ ...prev, subject: chosen.title }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [chosen?.id]);

    const handleSave = () => {
        console.log("Save", { pages, email, templateId });
        setOpenModal(true);
    };
    const handleContinue = () => navigate(`/campaign/review?template=${encodeURIComponent(templateId)}`);
    ...
    return (
        ...
        "
            ...
            <TemplateModalsave open={openModal}
