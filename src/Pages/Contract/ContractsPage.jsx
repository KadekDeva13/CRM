...
import { getContracts, createContract, updateContract, deleteContract } from "../../api/contract.mock";

const STATUS_FILTERS = ["", "Draft", "Sent", "Under Review", "Signed", "Declined", "Expired", "Prospect"];
const SORTS = [
...
export default function ContractsPage() {
  const [params, setParams] = useState({ search: "", page: 1, perPage: 8, sort: "updated_at:desc", status: "" });
  ...
  const onDelete = async (row) => {
    if (!confirm(`Hapus contract "${row.title}" (${row.number})?`)) return;
    try {
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button onClick={() => { setEditing(null); setOpenForm(true); }}>+ New Contract</Button>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-white/10 ring-1 ring-white/10 overflow-hidden">
        <table className="min-w-full text-sm text-white/90">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={8} className="px-4 py-6 text-center text-white/60">Loading…</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={8} className="px-4 py-6 text-center text-white/60">No contracts yet</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="border-t border-white/10">
                  ...
                  <td className="px-4 py-2"><ContractStatusBadge value={r.status} /></td>
                  <td className="px-4 py-2">{r.end_date || "-"}</td>
                  <td className="px-4 py-2">{r.owner_name || "-"}</td>
                  <td className="px-4 py-2 text-right">
                    <button className="px-2 py-1 rounded hover:bg-white/10" onClick={() => { setEditing(r); setOpenForm(true); }}>Edit</button>
                    <button className="ml-2 px-2 py-1 rounded hover:bg-white/10 text-red-300" onClick={() => onDelete(r)}>Delete</button>
                  </td>
                </tr>
              ))
            )}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 backdrop-blur">
          <div className="w-full max-w-2xl rounded-xl bg-neutral-900 ring-1 ring-white/10 p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-white text-lg">{editing ? "Edit Contract" : "New Contract"}</h3>
