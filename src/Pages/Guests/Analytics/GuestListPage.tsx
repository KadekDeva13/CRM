...
import StatusPill from "../../../components/StatusPill/StatusPillGuest";
...
export default function GuestPage() {
  return (
    <div className="space-y-6 bg-[#D2D4D3]">
      <div>
        <h1 className="text-2xl font-semibold text-zinc-600">Guest</h1>
        <p className="text-zinc-600">Central Hub of Viewing Arrival</p>
      </div>

      <div className="flex items-center justify-end gap-2">
        <input
          type="date"
          className="h-9 rounded-md border border-zinc-700 bg-[#D2D4D3] px-3 text-sm outline-none text-zinc-600"
        />
        <input
          type="date"
          className="h-9 rounded-md border border-zinc-700 bg-[#D2D4D3] px-3 text-sm outline-none text-zinc-600"
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-400 bg-[#D2D4D3] text-zinc-600">
        <div className="flex items-center justify-between border-b border-zinc-400 px-4 py-3 bg-zinc-300">
          <div className="text-sm font-medium text-zinc-600">Guest List</div>
          <div className="flex items-center gap-2">
            <button className="h-9 w-9 rounded-md bg-zinc-200 hover:bg-white/20 grid place-items-center" aria-label="Filter">
            ...
            <button className="h-9 w-9 rounded-md bg-zinc-200 hover:bg-white/20 grid place-items-center" aria-label="More">
              ⋮
            </button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="text-xs uppercase tracking-wide text-zinc-600 bg-zinc-300">
              <tr>
                ...
                <th className="px-5 py-2 text-justify-center font-medium">Status</th>
              </tr>
            </thead>

            <tbody className="divide-y divide-zinc-200">
              {dummyGuests.map((g) => (
                <tr key={g.id} className="bg-zinc-300">
                  <td className="px-5 py-3">
                    <AvatarName name={g.name} avatarUrl={g.avatarUrl} />
                  </td>
                  <td className="px-5 py-3">{fmtDate(g.lastStayDate)}</td>
                  <td className="px-5 py-3">{fmtMoney(g.offering)}</td>
                  <td className="px-5 py-3">
