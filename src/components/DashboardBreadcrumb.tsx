  ...
  if (inSetupFlow) {
    ...
    return (
      <div className="hidden items-center gap-2 md:flex text-sm">
        <svg className="text-zinc-500 " width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div key={c.to} className="flex items-center gap-2">
            {i > 0 && (
              <svg width="5" height="13" viewBox="0 0 5 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4.6733 0.340908L1.39205 12.5312H0.318182L3.59943 0.340908H4.6733Z" fill="#050505" />
              </svg>
            )}
            {i < fixed.length - 1 ? (
              <Link to={c.to} className="text-zinc-500 hover:underline">{c.label}</Link>
            ) : (
              <span className="text-zinc-500 font-medium truncate">{c.label}</span>
            )}
          </div>
        ))}
      </div>
