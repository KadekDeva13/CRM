...
export default function ContractStatusBadge({ value }) {
  const cls = MAP[value] || "bg-white/10 ring-white/20 text-white/80";
  return (
    <span className={`inline-flex px-2 py-0.5 rounded-md ring-1 text-[12px] ${cls}`}>
      {value || "-"}
    </span>
  );
}
