...
export default function AppSidebar({
  ...
  const GROUPS: MenuGroup[] = [
    { key: "overview", label: "Overview", icon: <IconOverview />, to: "/overeview" },
        ...
        <div className="pt-16 h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-3 text-sm text-white/70">
            <div className="mb-5 space-y-1">
              <SimpleNav to="/overeview" collapsed={collapsed} onClick={onCloseMobile}>
...
function NavGroup({
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          [
            "flex items-center justify-center rounded-md py-2",
            (isActive || isParentActive) ? "bg-white/10 text-white" : "text-white/80 hover:bg-white/5 hover:text-white",
          ].join(" ")
        }
      >
        {group.icon}
  ...
  const parentClasses = [
    "group flex items-center gap-3 rounded-full px-4 py-3 cursor-pointer",
    isParentActive ? "bg-white/10 text-white" : "text-white/80 hover:bg-white/5 hover:text-white",
  ].join(" ");
  ...
  const ParentRow = (
    ...
    >
      ...
      {hasChildren && (
        ...
        >
          <svg
            className={["h-4 w-4 transition-transform", isOpen ? "rotate-90" : ""].join(" ")}
            viewBox="0 0 20 20"
            fill="currentColor"
          >
  ...
  return (
    <div className="space-y-1">
      {group.to ? (
        <NavLink to={group.to} onClick={onAnyClick} className="block">
          {ParentRow}
        </NavLink>
      ) : (
        ParentRow
                ...
                {group.children!.map((c) => (
                  <li key={c.to}>
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        [
                          "flex items-center rounded-md px-3 py-2 text-[13px]",
                          isActive
