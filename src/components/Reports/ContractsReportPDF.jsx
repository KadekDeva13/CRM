...
const styles = StyleSheet.create({
  ...
  cell: { paddingVertical: 4, paddingHorizontal: 6, borderRight: 1, borderBottom: 1, borderColor: "#ccc" },
  ...
  footer: { position: "absolute", bottom: 20, left: 28, right: 28, fontSize: 9, color: "#666", textAlign: "right" },
});
...
const fmtDate = (d) => (d ? format(new Date(d), "dd MMM yyyy", { locale: id }) : "-");
...
export default function ContractsReportPDF({ data = [], filters = {} }) {
  ...
  const rangeStr =
    filters.startDate || filters.endDate
      ...
      : "All Dates";
  const statusStr = filters.status && filters.status !== "" ? filters.status : "All Statuses";
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page} wrap>
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>Range: {rangeStr} • Status: {statusStr}</Text>
        </View>

        <View style={styles.table}>
          <View style={[styles.row, styles.headRow]}>
            <Text style={[styles.cell, styles.center, styles.cNum]} wrap={false}>Number</Text>
            <Text style={[styles.cell, styles.center, styles.cTitle]}>Title</Text>
            <Text style={[styles.cell, styles.center, styles.cParty]}>Counterparty</Text>
            <Text style={[styles.cell, styles.center, styles.cVal]}>Value</Text>
            <Text style={[styles.cell, styles.center, styles.cStat]}>Status</Text>
            <Text style={[styles.cell, styles.center, styles.cEnd]} wrap={false}>End Date</Text>
          </View>
          ...
          {data.length === 0 ? (
            ...
            data.map((r) => (
              <View key={r.id} style={styles.row} wrap={false}>
                <Text style={[styles.cell, styles.cNum]} wrap={false}>{r.number}</Text>
                <Text style={[styles.cell, styles.cTitle]}>{r.title}</Text>
                <Text style={[styles.cell, styles.center, styles.cParty]}>{r.counterparty || "-"}</Text>
                <Text style={[styles.cell, styles.cVal, styles.right]} wrap={false}>
                  {fmtMoney(r.value, r.currency)}
                </Text>
                <Text style={[styles.cell, styles.center, styles.cStat]}>{r.status}</Text>
